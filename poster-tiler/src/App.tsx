...
const mmToPx = (mm: number, dpi: number) =>
  Math.round(mmToIn(mm) * clamp(dpi, LIMITS.dpi.min, LIMITS.dpi.max));
...
export default function App() {
  ...
  // raw strings so typing never glitches
  ...
  async function generatePDF() {
    ...
    const doc = new jsPDF({
      unit: "mm",
      format: [P.w, P.h],
      orientation: jsOrientation,
      compress: true,
    });

    const contentWmm = P.w - 2 * margin;
    const contentHmm = P.h - 2 * margin;
      ...
      <header className="border-b border-gray-200 bg-white dark:bg-slate-900 dark:border-slate-800">
        <div className="mx-auto max-w-6xl px-6 py-4 flex items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            <div className="h-8 w-8 rounded-xl bg-indigo-600/90 dark:bg-indigo-500 grid place-items-center text-white font-bold">
              PS
            </div>
